...
def uniformCostSearch(problem):
    ...
    while not fringe.isEmpty():
        (state, paths, priority) = fringe.pop()
        if problem.isGoalState(state):
            actions = paths
            return actions
        if state not in closed:
            closed.append(state)
            for child in problem.getSuccessors(state):
                fringe.update((child[2], paths + [child[0]], priority + child[1]), priority + child[1])
    return actions
    util.raiseNotDefined()
...
def aStarSearch(problem, heuristic=nullHeuristic):
    ...
    while not fringe.isEmpty():
        ...
        if state not in closed:
            closed.append(state)
            for child in problem.getSuccessors(state):
                newState = (child[2], paths + [child[0]], priority + child[1])
                fringe.update(newState, priority + child[1] + heuristic(child[2], problem))
    return actions
    util.raiseNotDefined()
