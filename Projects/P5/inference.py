...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for possiblePos in self.allPositions:
            self.beliefs[possiblePos] *= self.getObservationProb(observation, pacmanPos, possiblePos, jailPos)
        self.beliefs.normalize()
...
class ParticleFilter(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        pacmanPos = gameState.getPacmanPosition()
        jailPos = self.getJailPosition()
        for possiblePos in self.particles:
            belief[possiblePos] += self.getObservationProb(observation, pacmanPos, possiblePos, jailPos)
        belief.normalize()
        if belief.total() == 0:
            self.initializeUniformly(gameState)
        else:
...
class JointParticleFilter(ParticleFilter):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for particle in belief.keys():
            for i in range(self.numGhosts):
                belief[particle] *= self.getObservationProb(observation[i], pacmanPos, particle[i], self.getJailPosition(i))
        belief.normalize()
        if belief.total() == 0:
            self.initializeUniformly(gameState)
        else:
